...
Appointment.prototype.returnHTML = function() {
  return `<div>
              <table>
                ...
                <tr><th>Patient</th><td><a href='${this.patientUrl()}'>${
    this.patientName
  }</a></td></tr>
              </table>
              <a href='/appointments/${
                this.id
              }' class='show-page-button'>View Full Details</a>
              <hr>
            </div>`;
};
...
function getAndAppendAppointments() {
  $.getJSON("/appointments")
    ...
    .fail(function(jqXHR, textStatus, errorThrown) {
      const errorMessage =
        "getJSON request failed! " + textStatus + " - " + errorThrown;
      alert(errorMessage);
    });
}
...
$(function() {
  $("#quick-create-form").on("submit", function(e) {
    ...
    $.post(url, formData, function() {})
      .success($("#quick-create-form")[0].reset())
